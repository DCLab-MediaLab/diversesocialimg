...
function getUserCred($user){
    ...
    if (!isset($usercred[$user]))
    {
        ...
        $vis=is_null($x["credibilityDescriptors"]["visualScore"]) ? 0.1 : $x["credibilityDescriptors"]["visualScore"];
        $tagsum=is_null($x["credibilityDescriptors"]["tagSpecificity"]) ? 0.1 : $x["credibilityDescriptors"]["tagSpecificity"];
        
        $usercred[$user]=$vis*$tagsum;
    }
...
foreach($loc as $l1)
{
    ...
    foreach($d["photo"] as $photo)
    {
        ...
        $c*=(1-0.02*sqrt($photo["@attributes"]['rank']));
